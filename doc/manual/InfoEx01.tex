...
\begin{tabular}{ll}
  ...
  \texttt{type}       &  7 bytes \\
  delimiter           &  2 bytes \\
  \texttt{status}     &  3 bytes \\
  delimiter           &  2 bytes \\
  \texttt{name}       &  7 bytes \\
  delimiter           &  2 bytes \\
  \texttt{version}    &  5 bytes \\
  delimiter           &  2 bytes \\
  \texttt{copyright}  & 18 bytes \\
  delimiter           &  2 bytes \\
  \texttt{other}      & 10 bytes \\
  null                &  1 byte \\ \hline
  \textbf{Total}      & \textbf{67 bytes}
\end{tabular}

Note that we also store 7 additional bytes to hold the segment lengths (one byte
per segment).  If the total length of your string exceeds 255 bytes, it will be
14 bytes (two bytes per segment).

{\em dwmwhat} would print this when scanning a compiled program
